...
These include Naive Split Conformal Prediction, which uses absolute residuals; Conformalized Quantile Regression (CQR)~\citep{romano2019conformalized}, which learns conditional quantiles; Asymmetric CQR~\citep{sesia2020comparison}, which calibrates upper and lower bounds separately; Conformal Histogram Regression (CHR)~\citep{sesia2021conformal}, which uses histogram-based estimation; and Locally adaptive variance-based methods (LVD)~\citep{lin2021locally}. We also evaluate boosted variants of CQR and LCP~\citep{xie2024boosted}, as well as R2CCP~\citep{guha2024conformal}, which models the full conditional distribution via regression-as-classification.
...
Table~\ref{tab:rsg} and Figure~\ref{fig:decoupling} show that ranking quality and scoring precision are not coupled. High correlation does not imply narrow intervals.
...
Table~\ref{tab:conditional} shows coverage conditioned on ground truth score. Coverage is lowest for GT = 1 due to systematic overscoring, while other levels achieve near-perfect adjusted coverage.
...
Table~\ref{tab:mondrian} in the main paper shows results for Mondrian conformal prediction, which computes group-specific quantiles based on task difficulty. Compared to standard R2CCP, Mondrian CP produces narrower intervals for easier tasks and improved coverage for harder ones. Easy tasks receive 16.6\% narrower intervals, while hard tasks achieve higher coverage, reducing unnecessary conservativeness.
...
Table~\ref{tab:ablation_seeds} shows that R2CCP results stabilize by 5--10 seeds. Coverage and width means remain virtually unchanged from 5 to 30 seeds, with standard deviation decreasing modestly. Our choice of 10 seeds provides a reliable estimate with manageable compute.

\begin{table}[t]
\centering
\caption{R2CCP stability as a function of number of seeds (LLaVA-Critic-7B, MLLM-Judge). Results stabilize by 5--10 seeds.}
...
Table~\ref{tab:ablation_split} shows that R2CCP is robust to calibration/test split ratio. Coverage remains near the 90\% target across all splits, with width stable at ${\sim}3.05$. With a 30/70 split (smallest calibration set), coverage drops slightly to 88.7\% but remains close to target. The 50/50 split used throughout our experiments provides a balanced trade-off.
...
Table~\ref{tab:error_bins_all} extends the error-bin analysis to all three judges. The recovery pattern is consistent: boundary-adjusted coverage exceeds 99\% for $\pm 1$ and $\pm 2$ errors across all judges. Gemini achieves higher coverage for larger errors ($\pm 3$: 96.2\% vs.\ 91.5\% for LLaVA) because its narrower but better-centered intervals extend to the true score more often.

\begin{table}[t]
\centering
\caption{CP coverage (boundary-adjusted) by error magnitude across all 3 judges (R2CCP, 10 seeds).}
...
Table~\ref{tab:phi4_polaris} compares judges on the Polaris captioning benchmark. LLaVA-Critic-7B, trained specifically for evaluation, dramatically outperforms the general-purpose Phi-4-15B. With chain-of-thought reasoning, Phi-4 achieves only Pearson $= 0.128$ and R2CCP width $= 3.60$. Without reasoning mode, performance degrades further to Pearson $= 0.036$, as the model defaults to score 4 for 72\% of samples. This confirms that \textbf{evaluation specialization matters more than model scale}: a 7B specialist produces $5.3\times$ narrower conformal intervals than a 15B generalist on the same task.

\begin{table}[t]
\centering
\caption{Judge comparison on Polaris. LLaVA-Critic (7B, evaluation-specialized) vastly outperforms Phi-4 (15B, general-purpose) on captioning evaluation.}
