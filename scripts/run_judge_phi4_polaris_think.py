...
def load_dataset():
    """Load Polaris captioning dataset (balanced 924-sample subset)."""
    import pandas as pd
    import numpy as np
    df = pd.read_csv('data/polaris/polaris_test_processed.csv')

    # Load balanced indices (200 per GT score, balanced across difficulty)
    balanced_idx = pd.read_csv('data/polaris/balanced_1000_indices.csv')['original_idx'].tolist()
    df = df.iloc[balanced_idx].reset_index(drop=True)
