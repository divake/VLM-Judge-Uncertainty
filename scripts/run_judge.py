#!/usr/bin/env python3
"""Run judge VLM inference on actor outputs.

Usage:
    python scripts/run_judge.py --config configs/experiments/pilot_mllm_judge.yaml
    python scripts/run_judge.py --config configs/experiments/pilot_mllm_judge.yaml --limit 5
"""
...
def main():
    ...
    parser.add_argument("--use-dataset-answers", action="store_true", default=True,
                        help="Use answers from dataset (default for MLLM-Judge)")
    ...
    print(f"[run_judge] Actor outputs: {actor_output_dir}")
    ...
    model.load_model(device=exp_config["judge"].get("gpu", "cuda:0"))
